...
>
> Apple Developer 인증서가 없어 처음 실행 시 경고가 표시됩니다.
>
> **권장 — `_signed.zip` 파일 사용 (시스템 설정에서 허용):**
> 1. `_signed.zip`의 앱을 Applications 폴더로 이동
> 2. 앱을 처음 실행하면 "Not Opened" 다이얼로그 등장 → **Done** 클릭
> 3. `시스템 설정` → `개인 정보 보호 및 보안` → 하단 스크롤
> 4. "Claude Session Manager이(가) 차단되었습니다" → **그래도 열기** 클릭
> 5. Touch ID / 비밀번호 입력 후 실행
>
> **대안 — 터미널 1줄로 해결 (`.dmg` 파일 사용 시):**
> ```bash
